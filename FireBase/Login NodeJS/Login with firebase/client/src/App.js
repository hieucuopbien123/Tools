...
function App() {
	//xài 1 biến state để điều khiển front end và 1 biến localStorage để tối ưu hóa quá trình đăng nhập
	//nhưng như thế phải làm nút đăng xuất nx
	...
	useEffect(() => {
		...
		onAuthStateChanged(auth1, (userCred) => {
			if (userCred) {
				setAuth(true);
        //dùng localStorage để nó k bị nhảy vc load trang khi đã từng đăng nhập r mà nó sẽ check ở trong phiên đó 
        //thì truy cập được luôn
