...
void AthHandler::parseResponse(const QByteArray &response)
{
    ...
    if(jsonDocument.object().contains("error")){
        ...
        m_idToken = idToken; // Biến bất cứ giá trị nào lấy ở bấy cứ đâu thành global dùng cho mọi hàm của class bằng cách lưu như này
        qDebug() << m_idToken;
        emit userSignedIn();
        // Thay vì xử lý nhát gừng ta cho nó phát signal => bất cứ class nào mà muốn thay đổi gì khi user sign in thì bắt sẽ tốt hơn
    }else{
        qDebug() << "The responses were " << response;
    }
}
// SignIn hay SignUp thì cũng post cái tk, mk vào database trả lại idToken. Lấy idToken nhét vào url và get nó gửi lại user
